...
BROADCAST_MODE = os.getenv("BROADCAST_MODE", "users")  # groups | users | both
...
async def send_telegram_html(chat_id: int, text: str):
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
    ...
    if "my_chat_member" in data:
        ...
        if user["is_bot"] and user["id"] == BOT_ID:
            ...
            if status == "administrator":
                supabase.table("bot_groups").upsert({"chat_id": chat_id, "is_admin": True, "is_active": True}, on_conflict="chat_id").execute()
            elif status in ["member", "restricted"]:
                supabase.table("bot_groups").update({"is_admin": False}).eq("chat_id", chat_id).execute()
            elif status in ["left", "kicked"]:
                supabase.table("bot_groups").update({"is_active": False, "is_admin": False}).eq("chat_id", chat_id).execute()
        return {"status": "ok"}
...
async def season_broadcast():
    response = supabase.table("seasons").select("*").eq("is_active", True).eq("season_name", CURRENT_SEASON).execute()
    if not response.data:
        return {"status": "no active season"}
    season = response.data[0]
    season_id = season["id"]
    ...
    emoji_list = ", ".join(selected_emojis)
    prompt = """You are generating a Telegram broadcast message.
Context:
- Campaign name: "{{season_name}}"
- Giveaway ends in {{days_remaining}} days
- Use the following emojis: {{emoji_list}}
...
Return ONLY the message content in HTML."""
    prompt = prompt.replace("{{season_name}}", season_name).replace("{{days_remaining}}", str(days_remaining)).replace("{{emoji_list}}", emoji_list)
    genai.configure(api_key=GEMINI_API_KEY)
    model = genai.GenerativeModel('gemini-1.5-flash')
    response = model.generate_content(prompt)
    message = response.text.strip()
    if message.lower().startswith("html"):
        message = message[4:].strip()
    message_hash = hashlib.sha256(message.encode()).hexdigest()
    check = supabase.table("ai_messages_log").select("id").eq("season_id", season_id).eq("message_hash", message_hash).execute()
    if check.data:
        for _ in range(3):
            response = model.generate_content(prompt + " Please generate a completely different message.")
            message = response.text.strip()
            if message.lower().startswith("html"):
                message = message[4:].strip()
            message_hash = hashlib.sha256(message.encode()).hexdigest()
    ...
    supabase.table("ai_messages_log").insert({"season_id": season_id, "message_hash": message_hash, "created_at": now.isoformat()}).execute()
    targets = []
    mode = BROADCAST_MODE
    if mode in ['groups', 'both']:
        groups = supabase.table("bot_groups").select("chat_id").eq("is_admin", True).eq("is_active", True).execute()
        targets += [g["chat_id"] for g in groups.data]
    if mode in ['users', 'both']:
        users_res = supabase.table("users").select("user_id").eq("is_active", True).execute()
        targets += [u["user_id"] for u in users_res.data]
    for t in targets:
        try:
            resp = await send_telegram_html(t, message)
            if not resp.json().get("ok"):
                raise Exception(resp.json().get("description"))
        except Exception as e:
            print(f"Failed to send to {t}: {e}")
            if t < 0:
                supabase.table("bot_groups").update({"is_active": False}).eq("chat_id", t).execute()
            else:
                supabase.table("users").update({"is_active": False}).eq("user_id", t).execute()
